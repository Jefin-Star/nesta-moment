...
export const TrialBookingModal: React.FC<TrialBookingModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Small delay so user sees confirmation state before opening WhatsApp
    setTimeout(() => {
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl p-6 sm:p-8 z-10 text-slate-100 my-8"
          >
            {/* Close Button */}
            <button
              onClick={onClose}
